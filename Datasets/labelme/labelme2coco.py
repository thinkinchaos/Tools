...
class labelme2coco(object):
    ...
    def annotation(self, points, label, num):
        ...
        # voc['bbox'] = str(self.getbbox(points)) # 使用list保存json文件时报错（不知道为什么）
        # list(map(int,a[1:-1].split(','))) a=voc['bbox'] 使用该方式转成list
...
def _main():
    # args = parser()
    labelme_jsons = glob.glob('//192.168.133.15/workspace/sunxin/DATA/zhatu/zhatu0814_api/V1.1' + '/*.json')
    random.shuffle(labelme_jsons)
    split_pos = int(len(labelme_jsons) * 0.9)

    print('transfer to train data:')
    labelme2coco(labelme_jsons[:split_pos], os.path.join('D:/', 'train.json')).save_json()

    print('transfer to train test-env:')
    labelme2coco(labelme_jsons[split_pos + 1:], os.path.join('D:/', 'val.json')).save_json()


def do_test_coco():
    # args = parser()
    json_file = 'D:/train.json'
    image_root = '//192.168.133.15/workspace/sunxin/DATA/zhatu/zhatu0814_api/image'
    image_save = 'D:/'
    ...
    for imgInf in tqdm(imgInfs):
        ...
        for ann in anns:
            bbox = ann['bbox']
            # print(bbox)
            # cv2.rectangle(I, (int(bbox[0]), int(bbox[1])), (int(bbox[2] - bbox[0]), int(bbox[3] - bbox[1])), (255, 0, 0), 2)
